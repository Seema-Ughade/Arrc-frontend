...
const MainLayout = () => {
  ...
  useEffect(() => {
    gsap.fromTo(
      ['#service', '#about', '#processSection'],
      { opacity: 0, y: 50 },
      { opacity: 1, y: 0, duration: 1, stagger: 0.3, ease: 'power2.out' } // Smoother easing
    );
  }, []);

  // Smooth scrolling function with faster duration
  const scrollToSection = (sectionId) => {
    gsap.to(window, { 
      scrollTo: `#${sectionId}`, 
      duration: 0.8,  // Faster scroll duration
      ease: 'power2.inOut' // Smooth easing for better flow
    });
  };
